...
RUN echo "Installing additional software" \
    ...
    && yum -y update  \
    && yum -y upgrade \
    && yum -y install --nogpgcheck k6 \
    && yum -y install \
       yum-utils device-mapper-persistent-data lvm2 sudo \
       docker-ce-cli oci-runtime conntrack-tools torsocks iptables   \
       ...
       python37 gcc python3-devel sshuttle  bash zsh procps rsync mc htop ansible findutils jq bzip2  \
    ...
    && rpm -ivh https://download.opensuse.org/repositories/devel:/kubic:/libcontainers:/stable/CentOS_7/x86_64/skopeo-1.2.2-2.el7.3.1.x86_64.rpm --nodeps \
...
#install terragrunt
RUN curl -sL "https://github.com/gruntwork-io/terragrunt/releases/download/v0.29.10/terragrunt_linux_amd64" -o terragrunt_linux_amd64 \
...
#kapp
RUN curl -sLO "https://github.com/k14s/kapp/releases/download/v0.37.0/kapp-linux-amd64" && \
    mv kapp-linux-amd64 /usr/bin/kapp && \
    chmod +x /usr/bin/kapp

#ytt
RUN curl -sLO "https://github.com/k14s/ytt/releases/download/v0.34.0/ytt-linux-amd64" && \
    mv ytt-linux-amd64 /usr/bin/ytt && \
    chmod +x /usr/bin/ytt

#tekton
RUN curl -sLO "https://github.com/tektoncd/cli/releases/download/v0.19.0/tkn_0.19.0_Linux_x86_64.tar.gz" && \
    tar xvzf tkn_0.19.0_Linux_x86_64.tar.gz -C /usr/bin tkn && \
    rm -rd tkn_0.19.0_Linux_x86_64.tar.gz && \
    chmod +x /usr/bin/tkn

#vault
RUN curl -sLO "https://releases.hashicorp.com/vault/1.7.2/vault_1.7.2_linux_amd64.zip" && \
...
RUN echo "Install JAVA MAVEN" \
    ...
    && zsh -c 'source "/root/.sdkman/bin/sdkman-init.sh" && sdk install gradle 7.0.2' \
